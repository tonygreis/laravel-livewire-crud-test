                    ...
                    <div class="flex-shrink w-full inline-block relative">
                        <select wire:model.lazy="parent" class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded">
                            ...
                            @foreach($categories as $cat)
                                <option name="parent" value="{{ $cat->id }}">{{ $cat->name }}</option>
                            @endforeach
                        </select>
                        ...
                        <div class="flex-shrink w-full inline-block relative">
                            <select class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded">
                                ...
                                @foreach($children as $child)
                                    <option name="category_id" value="{{ $child->id }}">{{ $child->name }}</option>
                                @endforeach
                            </select>
                            <div class="pointer-events-none absolute top-0 mt-3  right-0 flex items-center px-2 text-gray-600">
                                <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                        ...
                        <div class="-mx-3 md:flex mb-6">
                            ...
                            <div class="w-full px-3">
                                <label class="block text-gray-500 font-bold" for="remember">
                                    <input class="ml-2 leading-tight" type="radio" id="{{ $field->id }}" name="{{ $field->id }}">
                                    <span class="text-sm">
                                           {{ $field->name }}
                                        </span>
                                </label>
                            ...
                            <div class="w-full px-3">
                                @foreach($field->options as $op)
                                    <label class="block text-gray-500 font-bold">
                                        <input class="ml-2 leading-tight" type="checkbox" id="remember" name="{{ $field->id }}" value="{{ $op->id }}">
                                        <span class="text-sm">
                                           {{ $op->value }}
                                        </span>
                                    </label>
